...
class RoomRateSubCategoryController extends Controller
{
    ...
    {
        $this->mainTableName = 'cfg_init_room_rate_sub_category';
        $this->joinTable1 = 'cfg_init_room_rate_category';
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            DB::select('call insert_' . $this->mainTableName . '(?,?,?,?,?)', [
                $request->code, $request->name, $request->category_code, $request->id_sort, $request->user_id
            ]);
    ...
    {
        $query=RoomRateSubCategory::find($id);
    ...
    public function delete(Request $request)
    {
        ...
        $room = RoomRate::where('sub_category_code', $request->code)->first();
        if (!$room) {
    ...
    public function dataAgGrid(Request $request)
    {
        ...
        $model = RoomRateSubCategory::select(
    ...
    public function dataLookUp(){
        $category= RoomRateCategory::select('code','name')->get();
