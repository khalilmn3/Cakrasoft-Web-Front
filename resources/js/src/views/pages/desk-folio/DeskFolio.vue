<template>
    <div class="guest-in-house">
        <navigation-header @showModal="showForm" :insertButton="true" @refreshData="(val)=>{limitPageSize=parseFloat(val);$refs.filterHeader.onRefreshData(val)}"></navigation-header>
        <filter-header ref="filterHeader" :formType="global.formType.deskFolio" @search="refreshData" :limitPageSize="limitPageSize" :searchOption="searchFilterData" :searchDefault="vModelDefaultSearchData"></filter-header>
...
<!-----------start need setting manual---------------->
    <!----------- START POPUP MAIN -------------->
        <folio-form ref="folioForm" :titleForm="this.$t('folio.title.deskFolio')" :formType="global.formType.deskFolio" @refreshData="$refs.filterHeader.onRefreshData()"/>
    <!----------- END POPUP MAIN ---------------->
...
export default {
    ...
    beforeMount() {
        ...
        this.columnDefs = [
            ...
            { headerName: this.$t('folio.table.isLockIcon'), field: 'is_lock', enableRowGroup: false, resizable: false, filter: false, suppressMenu: true, sortable: false, width: 30, cellStyle: { textAlign: 'center' }, cellRenderer: 'iconLockRenderer' },
