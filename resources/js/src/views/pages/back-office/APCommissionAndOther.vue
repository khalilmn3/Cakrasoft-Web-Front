<template>
    <div>
        <navigation-header insertButton @showModal="showModal" @refreshData="(val)=>{limitPageSize=parseFloat(val);$refs.filterHeader.onRefreshData(val)}">{{ $t('apPayment.insertPayment') }}</navigation-header>
        ...
        <vx-card id="main-container" class="vs-con-loading__container">
            ...
            <vs-col class="w-full md:w-1/3">
                <span class="p-2">{{ $t('apPayment.payment') }}</span>
                <ag-grid-vue
                    :style="global.styleAgGridFrame"
                    :class="global.themeAgGrid"
                    pagination="true"
        ...
        <APRefundDepositPaymentForm ref="apRefundDepositPaymentForm" :formType="formType" @refreshData="$refs.filterHeader.onRefreshData()"/>
...
import APRefundDepositPaymentForm from './components/APRefundDepositPaymentForm';
...
export default {
    ...
    data() {
        return {
            ...
            searchFilterData: [
                ...
                { text: this.$t('common.filter.roomNumber'), value: 1 },
                { text: this.$t('common.filter.guestName'), value: 2 },
                { text: this.$t('common.filter.remark'), value: 3 },
                { text: this.$t('common.filter.insertBy'), value: 4 },
            ],
            // query data search
            limitPageSize: 0,
            search: {
    ...
    beforeMount() {
        ...
        this.columnDefs = [
            { headerName: this.$t('common.table.action'), field: 'index', enableRowGroup: false, resizable: false, filter: false, suppressMenu: true, sortable: false, cellRenderer: 'actionGrid', colId: 'params', width: 70,},
            { headerName: this.$t('apPayment.table.account'), field: 'Account', width: 200 },
            { headerName: this.$t('apPayment.table.company'), field: 'company', width: 250 },
            { headerName: this.$t('apPayment.table.folioNumber'), field: 'folio_number', width: 120 },
            { headerName: this.$t('apPayment.table.roomNumber'), field: 'room_number', width: 130 },
            { headerName: this.$t('apPayment.table.guestName'), field: 'GuestName', width: 250 },
            { headerName: this.$t('apPayment.table.id'), field: 'id_log', width: 70 },
            { headerName: this.$t('apPayment.table.amount'), field: 'Amount', width: 120, cellStyle: { textAlign: 'right' }, valueFormatter: formatNumber, },
            { headerName: this.$t('apPayment.table.totalPaid'), field: 'TotalPaid', width: 140, cellStyle: { textAlign: 'right' }, valueFormatter: formatNumber, },
            { headerName: this.$t('apPayment.table.outstanding'), field: 'Outstanding', width: 120, cellStyle: { textAlign: 'right' }, valueFormatter: formatNumber, },
            { headerName: this.$t('apPayment.table.auditDate'), field: 'audit_date', width: 110, valueFormatter: formatDate, },
            { headerName: this.$t('apPayment.table.arrivalDate'), field: 'ArrivalDate', width: 110, valueFormatter: formatDate, },
            { headerName: this.$t('apPayment.table.departureDate'), field: 'DepartureDate', width: 130, valueFormatter: formatDate, },
            { headerName: this.$t('apPayment.table.remark'), field: 'remark', width: 370 },
            { headerName: this.$t('common.table.insertBy'), field: 'insert_by', width: 120 },
        ];
        this.columnDefs2 = [
            ...
            { headerName: this.$t('apPayment.table.refNumber'), field: 'ref_number', width: 110 },
            { headerName: this.$t('apPayment.table.date'), field: 'date', width: 110, valueFormatter: formatDate, },
            { headerName: this.$t('apPayment.table.amount'), field: 'amount', width: 120, cellStyle: { textAlign: 'right' }, valueFormatter: formatNumber, },
            { headerName: this.$t('apPayment.table.account'), field: 'JournalAccount', width: 200 },
            { headerName: this.$t('apPayment.table.remark'), field: 'remark', width: 150 },
    ...
    methods: {
        ...
        async getData(params) {
            try {
                ...
                const { data } = await apCommissionAndOtherResource.list(params);
                this.rowData = data;
                this.rowData2 = [];
        ...
        async getPaymentDetail(params) {
            const { data } = await apCommissionAndOtherResource.getPaymentList({id: params.id_log});
            this.rowData2 = data;
        },

        async showModal(idData, modeData) {
            this.$refs.apRefundDepositPaymentForm.showModal(idData, modeData);
        },

        handleUpdateEditor(idData) {
            this.$refs.apRefundDepositPaymentForm.showModal(idData, this.global.modeData.edit);
        },

        handleDeleteEditor(params) {
        ...
        async deleteData(params) {
            try{
                this.$loadingIndicator(this);
                const { data } = await apCommissionAndOtherResource.deletePayment({ref_number: params.ref_number, user_id: this.global.userInfo.code});
